...
**DEF 2.2.2.** In generale, se $A \in M_{m,p}(\mathbb{R})$ e $B \in M_{p, n}(\mathbb{R})$ allora definiamo il **prodotto** $A \cdot B$ come la *matrice $m \times n$* la cui *entrata di posto $ij$* è data dalla seguente: $$(A\cdot B)_{ij} := A_{(i)} \cdot B^{(i)} = \sum_{i=1}^p a_{1i}b_{i1}$$
